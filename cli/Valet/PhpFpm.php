...
class PhpFpm
{
    ...
    function fix(){
        ...
        $this->cli->runAsUser('brew list | grep php56- | xargs brew uninstall');
        info('Removing all old php70- packages from homebrew/php tap');
        $this->cli->runAsUser('brew list | grep php70- | xargs brew uninstall');
        info('Removing all old php71- packages from homebrew/php tap');
        $this->cli->runAsUser('brew list | grep php71- | xargs brew uninstall');
        info('Removing all old php72- packages from homebrew/php tap');
        $this->cli->runAsUser('brew list | grep php72- | xargs brew uninstall');

        info('Removing all old n98-magerun packages from homebrew/php tap');
        $this->cli->runAsUser('brew list | grep n98-magerun | xargs brew uninstall');

        info('Removing drush package from homebrew/php tap');
        $this->cli->runAsUser('brew list | grep drush | xargs brew uninstall');
        ...
        $this->cli->runAsUser('brew uninstall php56');
        info('Trying to remove php70...');
        $this->cli->runAsUser('brew uninstall php70');
        info('Trying to remove php71...');
        $this->cli->runAsUser('brew uninstall php71');
        info('Trying to remove php72...');
        $this->cli->runAsUser('brew uninstall php72');

        // If the current php is not 7.1, link 7.1.
        info('Installing and linking new PHP homebrew/core version.');
        $this->cli->runAsUser('brew uninstall ' . Brew::PHP_V71_FORMULAE);
        $this->cli->runAsUser('brew install ' . Brew::PHP_V71_FORMULAE);
        $this->cli->runAsUser('brew unlink '. Brew::PHP_V71_FORMULAE);
        $this->cli->runAsUser('brew link '.Brew::PHP_V71_FORMULAE.' --force --overwrite');
