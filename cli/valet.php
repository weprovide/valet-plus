...
if (is_dir(VALET_HOME_PATH)) {
    ...
    $app->command(
        ...
        function ($action, $name, $secure, $proxy) {
            if ($action === 'list') {
            ...
            if ($action === 'remove') {
                ...
                info(
                    'Current working directory unlinked from ' .
                    $name .
                    '.' .
                    basename(getcwd())
                );
    ...
    $app->command(
        ...
        function ($domain = null) {
            ...
            info(
                'The [' .
                $url .
                '] site has been secured with a fresh TLS certificate.'
            );
        }
    ...
    $app->command(
        ...
        function () {
            ...
            $driver =
                ValetDriver::assign(getcwd(), basename(getcwd()), '/', true);

            if ($driver) {
                info('This site is served by [' . get_class($driver) . '].');
            } else {
                warning(
                    'Valet could not determine which driver to use for this site.'
                );
            }
        }
    )->descriptions(
    ...
    $app->command(
        ...
        function ($services) {
            ...
            if (!empty($services)) {
                ...
                $phpVersions =
                    array_keys(\Valet\PhpFpm::SUPPORTED_PHP_FORMULAE);
    ...
    $app->command(
        ...
        function ($service, $targetVersion) {
            $supportedServices = [
                'php' => 'php',
            ...
            $service =
                (isset($supportedServices[$service]) ?
                    $supportedServices[$service] : false);
            ...
            switch ($service) {
                ...
                default:
                    throw new Exception(
                        'Service to switch version of not supported. Supported services: ' .
                        implode(
                            ', ',
    ...
    $app->command(
        ...
        function ($input, $output, $run, $name, $optional) {
            ...
            if ($run === 'pwd' || $run === 'password') {
                if (!$name || !$optional) {
                    throw new Exception(
                        'Missing arguments to change root user password. Use: "valet db pwd <old> <new>"'
                    );
                }
    ...
    $app->command(
        ...
        function ($input, $mode) {
            ...
            if (!in_array($mode, $modes)) {
                throw new Exception(
                    'Mode not found. Available modes: ' . implode(', ', $modes)
                );
            }
            ...
            if (Pecl::isEnabled('xdebug') === false &&
                ($mode === 'on' || $mode === 'enable')) {
    ...
    $app->command(
        ...
        function ($mode) {
            ...
            if (PeclCustom::isEnabled('ioncube_loader_dar') === false &&
                ($mode === 'on' || $mode === 'enable')) {
                ...
                info(
                    "[PECL-CUSTOM] ioncube_loader_dar extension is already installed"
                );
            }

            if (PeclCustom::isEnabled('ioncube_loader_dar') === true &&
                ($mode === 'off' || $mode === 'disable')) {
                ...
                info(
                    "[PECL-CUSTOM] ioncube_loader_dar extension is already uninstalled"
                );
            }
        }
    ...
    $app->command(
        ...
        function ($mode) {
            ...
            if (PhpFpm::linkedPhp() == '5.6') {
                throw new Exception(
                    'Memcache needs php 7.0 or higher, current php version: 5.6'
                );
            }
    ...
    $app->command(
        'proxy [url]',
        function ($input, $output, $url = null) {
            $url =
                ($url ?: Site::host(getcwd())) .
                '.' .
                Configuration::read()['domain'];
            $helper = $this->getHelperSet()->get('question');
            $question =
                new Question('Where would you like to proxy this url to? ');
    ...
    $app->command(
        ...
        function ($url = null) {
            ...
            info(
                "The [$url] will no longer proxy traffic and will use the Valet driver instead."
            );
        }
    )->descriptions(
        'Disable proxying for a site re-instating handling with a Valet driver.'
    );
    ...
    $app->command(
        ...
        'Open the logs for the specified service. (php, php-fpm, nginx, mysql, mailhog, redis)'
    );
}
