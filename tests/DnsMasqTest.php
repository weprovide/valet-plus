...
class DnsMasqTest extends TestCase
{
    ...
    public function testInstallPlacesConfigurationFilesInProperLocations()
    {
        ...
        $this->assertSame(
            'nameserver 127.0.0.1' . PHP_EOL,
            file_get_contents(__DIR__ . '/output/resolver/test')
        );

        $this->assertSame(
            'address=/.test/127.0.0.1' . PHP_EOL,
            file_get_contents(__DIR__ . '/output/custom-dnsmasq.conf')
        );
...
',
    ...
    public function testUpdateDomainRemovesOldResolverAndReinstalls()
    {
        $cli = Mockery::mock(CommandLine::class);
        $cli->shouldReceive('quietly')->with('rm /etc/resolver/old');

        $dnsMasq = Mockery::mock(
            DnsMasq::class . '[install]',
            [resolve(Brew::class), $cli, new Filesystem()]
        );
